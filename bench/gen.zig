...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    // Padding inflates each request with a filler header, so two clients can
    // send the same NUMBER of requests while consuming very different numbers
    // of BYTES. That is what separates byte-fairness from request-fairness.
    const pad: usize = if (argc > 7) try std.fmt.parseInt(usize, argv[7], 10) else 0;

    const rb = try gpa.alloc(u8, 128 + pad);
    const filler = try gpa.alloc(u8, pad);
    @memset(filler, 'p');
    const req = if (pad == 0)
        try std.fmt.bufPrint(rb, "GET /work/{d} HTTP/1.1\r\nHost: x\r\n\r\n", .{units})
    else
        try std.fmt.bufPrint(rb, "GET /work/{d} HTTP/1.1\r\nHost: x\r\nX-Pad: {s}\r\n\r\n", .{ units, filler });
    ...
    // Connection establishment is REPORTED, not assumed.
    //
    // The previous version returned error.Connect on the first failure, so a
    // client that lost connections in a simultaneous connect storm exited
    // early having served a fraction of the load -- and read as "starved" in a
    // fairness test. Two identical trials produced 51% and 0.1% shares purely
    // from this. Retry with backoff, count what actually established, and let
    // the caller reject a run that came up short.
    ...
    for (0..nconn) |i| {
        ...
        while (attempt < 50) : (attempt += 1) {
            ...
            if (!sysErr(linux.connect(conns[i].fd, @ptrCast(&addr), @sizeOf(sockaddr_in)))) break;
            _ = linux.close(conns[i].fd);
            conns[i].fd = -1;
            var ts = linux.timespec{ .sec = 0, .nsec = 2_000_000 };
    ...
    std.debug.print("GEN,{d},{d},{d},{d},{d},{d},{d},{d},{d},{d},{d},{d},{d}\n", .{
