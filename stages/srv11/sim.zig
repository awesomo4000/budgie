...
const Sim = struct {
    ...
    fn run(sm: *Sim, until_ns: i64, cfg: Cfg) void {
        ...
        for (1..sm.n_tasks + 1) |i| {
            ...
            sm.s.setPrio(t, 2);
            sm.s.assignSup(t, sup_conn);
            sm.s.admit(t, 0, cfg.reserve);
        ...
        while (sm.clock.v_ns < until_ns) {
            ...
            if (sm.s.nextRefillMs(now_ms)) |d| next_ns = @min(next_ns, sm.clock.v_ns + d * 1_000_000);
    ...
    fn step(sm: *Sim, t: TaskId, cfg: Cfg, r: std.Random) void {
        ...
        switch (tk.phase) {
            ...
            .working => {
                ...
                if (charge > 0 and !sm.s.chargeTo(t, .work, charge)) {
                    sm.budget_denials += 1;
                    tk.phase = .parked;
                    sm.s.disarm(t);
                    sm.s.arm(t, sm.clock.ms() + cfg.idle_deadline_ms);
...
fn once(seed: u64, n_tasks: u32, virt_s: i64, cfg: Cfg) Sim {
    var sm = Sim{
    ...
    sm.s.defineSup(sup_root, sched.no_sup, sched.unlimited, 100, "root");
    sm.s.defineSup(sup_conn, sup_root, if (cfg.conn_quota > 0) cfg.conn_quota else sched.unlimited, 100, "conn");
...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    const w0 = wallNs();
    var a = once(seed, ntask, virt_s, .{});
    const wall_a: u64 = @intCast(@max(1, wallNs() - w0));
    var b = once(seed, ntask, virt_s, .{});
    var c = once(seed +% 1, ntask, virt_s, .{});
    var d = once(seed, ntask, virt_s, .{ .quantum = 100 });
    var e = once(seed, ntask, virt_s, .{ .conn_quota = 20000 });
    var f = once(seed, ntask, virt_s, .{ .conn_quota = 20000 });
