...
const Knobs = struct {
    ...
    /// 1 = submit the response through the ring (costs a completion round
    /// trip per response). 0 = plain write(2), which completes inline for
    /// 74ns on this box.
    async_send: i64 = 1,
    defer_taskrun: i64 = 1,
    coop_taskrun: i64 = 0,
...
const App = struct {
    ...
    fn onData(a: *App, t: TaskId, c: *Conn, bytes: []const u8) void {
        ...
        if (bytes.len > room) {
            // The peer is pipelining deeper than the buffer allows. That is an
            // admission decision, not a silent truncation.
            a.stash_overflow += 1;
            return a.enterCleanup(t, c, .bad_request);
        }
...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    const a = &app_storage;
    @import("reactor_uring2.zig").defer_taskrun = K.defer_taskrun != 0;
    @import("reactor_uring2.zig").coop_taskrun = K.coop_taskrun != 0;
    try a.r.init(@intCast(K.io_bufs), @intCast(K.uring_buf_size));
    a.r.cqe_batch = @intCast(K.uring_cqe_batch);
    @import("reactor_uring2.zig").use_gen = K.gen_keys != 0;
    ...
    std.debug.print("data: on_data={d} dropped={d} parsed={d} pipelined_kept={d} stashed={d} stash_overflow={d}\n", .{ a.on_data, a.data_dropped, a.reqs_parsed, a.pipelined_kept, a.data_stashed, a.stash_overflow });
    ...
    std.debug.print("uring: enters={d} cqes={d} recv_arms={d} multishot_ended={d} stale_completions={d} enobufs={d} bytes_in={d}\n", .{
        a.r.enters, a.r.cqes_total, a.r.recv_arms, a.r.multishot_reups, a.r.stale_completions, a.r.enobufs, a.r.bytes_in,
    });
