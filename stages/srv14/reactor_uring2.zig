...
pub const Reactor = struct {
    ...
    /// Buffers are recycled inside `wait` the moment their bytes are copied
    /// out, so there is nothing left to release here.
    ...
    pub fn wait(r: *Reactor, s: *sched.Sched, timeout_ms: i32) usize {
        ...
        for (cqes[0..n]) |cqe| {
            ...
            // Copy out and recycle immediately: the completion's bytes are
            // exactly one whole buffer, so there is no shared head to get
            // wrong and nothing to hold across the rest of the batch.
            ...
            r.recycle(cqe);
            r.bytes_in += take;
            r.push(.{ .task = task, .kind = .data, .data = slice, .has_buf = false });
        }
        return r.done_n;
    }
