...
const Reactor = @import("reactor_uring2.zig").Reactor;
...
const App = struct {
    ...
    fn step(a: *App) void {
        ...
        // 3a. (after wait) completions are drained below.
        ...
        // Completion mode: the kernel already did the reads. Feed the bytes
        // straight into the parser and hand the buffer back to the ring.
        while (a.r.next()) |comp| {
            a.onCompletion(comp);
            a.r.release(comp);
        }
    ...
    fn park(a: *App, t: TaskId, c: *Conn, i: @import("reactor_uring2.zig").Interest) void {
        a.r.watch(t, c.fd, i);
    }
    ...
    fn onCompletion(a: *App, comp: @import("reactor_uring2.zig").Completion) void {
        const t = comp.task;
        if (!a.live_conn[t]) return;
        const c = &a.conn_store[t];
        switch (comp.kind) {
            .eof, .err => a.finish(t, c, .peer_gone),
            .writable => a.s.makeRunnable(t, .io),
            .data => a.onData(t, c, comp.data),
        }
    }

    fn onData(a: *App, t: TaskId, c: *Conn, bytes: []const u8) void {
        a.on_data += 1;
        if (c.phase != .reading) {
            a.data_dropped += 1;
            return;
        ...
        var off: usize = 0;
        while (off < bytes.len) {
            const used = b.parser.feed(bytes[off..]);
            if (used == 0 and b.parser.poll() == .need_input) break;
            off += used;
            const units = @divTrunc(@as(i64, @intCast(used)), 64) + 1;
            if (!a.s.chargeTo(t, .parse, units)) return a.enterCleanup(t, c, .budget_exhausted);
            switch (b.parser.poll()) {
                .need_input => continue,
                .protocol_error => return a.enterCleanup(t, c, .bad_request),
                .request => |req| {
                    a.reqs_parsed += 1;
                    c.work_left = req.work_units;
                    c.phase = .working;
                    a.s.makeRunnable(t, .spawn);
                    return; // remaining bytes stay in the kernel buffer; the
                            // pipelined case is handled on the next completion
                },
            }
        }
        a.s.arm(t, nowMs() + K.idle_deadline_ms);
    ...
    fn stepWriting(a: *App, t: TaskId, c: *Conn) void {
        ...
        a.s.arm(t, nowMs() + K.idle_deadline_ms);
        // Nothing pending: give the buffer back. An idle connection holds no
        // userspace buffer AND no kernel read buffer -- the ring only hands
        // one out when bytes actually arrive.
        a.releaseIfIdle(c);
        if (!a.r.watching(t)) a.r.armRecv(t, c.fd);
    }
...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    const a = &app_storage;
    try a.r.init(@intCast(K.io_bufs), 2048);
    ...
    std.debug.print("data: on_data={d} dropped={d} parsed={d}\n", .{ a.on_data, a.data_dropped, a.reqs_parsed });
    std.debug.print("uring: enters={d} sqes={d} recv_arms={d} multishot_ended={d} enobufs={d} bytes_in={d}\n", .{
        a.r.enters, a.r.sqes_total, a.r.recv_arms, a.r.multishot_reups, a.r.enobufs, a.r.bytes_in,
    });
