...
const Shared = struct {
    ...
    /// Monotonic ns at which the OLDEST undrained signal arrived. Deliberately
    /// 64-bit: a 128-bit atomic lowers to a compiler-rt libcall that is not
    /// async-signal-safe, and calling it from `onTick` segfaults. Monotonic ns
    /// in an i64 covers ~292 years of uptime.
    arrived_ns: std.atomic.Value(i128) = .init(0),
...
fn onTick(_: posix.SIG) callconv(.c) void {
    ...
    if (shared.pending.fetchAdd(1, .acq_rel) == 0) {
        shared.arrived_ns.store(@as(i128, nowNs()), .monotonic);
    }
}
...
const Kernel = struct {
    ...
    fn step(k: *Kernel) StepInfo {
        ...
        // under-report it.
        const arrived: i64 = @intCast(shared.arrived_ns.load(.monotonic));
